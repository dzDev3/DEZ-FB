```text
  ██████╗██████╗  █████╗  ██████╗██╗  ██╗
 ██╔════╝██╔══██╗██╔══██╗██╔════╝██║ ██╔╝
 ██║     ██████╔╝███████║██║     █████╔╝ 
 ██║     ██╔══██╗██╔══██║██║     ██╔═██╗ 
 ╚██████╗██║  ██║██║  ██║╚██████╗██║  ██╗
  ╚═════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝
         Multi-Threaded FB Crack - Author by Dezz
```

# DEZ-FB v1.0

Tool buat dump and crack akun Facebook. Udah pake engine GraphQL and Bloks.

### Fitur
* **Identity Engine**: Random 200+ device (Samsung, Pixel, Xiaomi, dll). Random OS Android 10-15.
* **Dumper**: Sikat member grup and reaksi post lewat GraphQL. Udah support pagination end-cursor.
* **Auto Pass**: Generator sandi otomatis (Nama Depan/Lengkap + 123, Angka 01-30, atau Custom Suffix).
* **Operation**: Bisa langsung crack dari hasil dump (pake Ctrl+C).
* **Login Bloks**: Pake payload 3-lapis mirip FB asli biar gak gampang CP.

### Install & Run
```bash
pip install requests
python dezfb1.0.py
```

### Note
- Butuh cookie FB pas awal jalan.
- Result kesimpan di folder `/Results`.
- Cookie kesimpan di `saved_cookie.txt`.

**Author: dzDev3**
